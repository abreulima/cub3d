...
int handle_key(int key, t_game *game)
{
	...
	if (key == XK_w)
	{
	    snprintf(message, MAXLINE, "%s;W", game->current_player);
	    sendto(game->network.sockfd, message, strlen(message), 0, (struct sockaddr*)NULL, sizeof(game->network.servaddr));
	}

	if (key == XK_a)
	{
	    snprintf(message, MAXLINE, "%s;A", game->current_player);
	    sendto(game->network.sockfd, message, strlen(message), 0, (struct sockaddr*)NULL, sizeof(game->network.servaddr));
	}

	if (key == XK_s)
	{
	    snprintf(message, MAXLINE, "%s;S", game->current_player);
	    sendto(game->network.sockfd, message, strlen(message), 0, (struct sockaddr*)NULL, sizeof(game->network.servaddr));
	}

	if (key == XK_d)
	{
	    snprintf(message, MAXLINE, "%s;D", game->current_player);
	    sendto(game->network.sockfd, message, strlen(message), 0, (struct sockaddr*)NULL, sizeof(game->network.servaddr));
	}
...
int main(int argc, char **argv)
{
	...
	if (strcmp(argv[1], "ONE") == 0)
		game.current_player = strdup("PLAYER_ONE");
	else
		game.current_player = strdup("PLAYER_TWO");
